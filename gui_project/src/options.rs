...
pub fn show_options_ui(app: &mut MyApp, ctx: &egui::Context, panel_frame: Frame) {
    ...
    egui::CentralPanel::default().frame(panel_frame).show(ctx, |ui| {
        ...
        //content_rect = content_rect.shrink(8.0);    //NON SO SE HO SBAGLIATO A COMMENTARE QUESTA MA NON SEMBRAVA UTILE

        // Define New capture button and Save button aligned horizontally
        let modificatore_screenshot = &app.modifier_sh_tmp;
        let tasto_screenshot = &app.code_sh_tmp;
        ui.vertical(|ui| {
            ui.horizontal(|ui| {
            ...
            let modificatore_save = &app.modifier_save_tmp;
            let tasto_save = &app.code_save_tmp;

            ui.label(format!(
                "                  {} + {}                             {}+{}",
                modificatore_screenshot, tasto_screenshot, modificatore_save, tasto_save
            ));
            ui.separator();
