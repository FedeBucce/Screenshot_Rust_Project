...
use egui::{Spinner, Shape, Color32, Stroke};
use ::egui::{TextureHandle, ColorImage};
use eframe::egui::ViewportCommand;
use std::{sync::{Arc, Mutex}, path::PathBuf, process::exit, thread, time::Duration};
...
pub fn register_hotkey_listener(my_app_instance: Arc<Mutex<MyApp>>) -> std::thread::JoinHandle<Result<(), String>> {
        ...
        loop {
            if let Ok(event) = GlobalHotKeyEvent::receiver().try_recv() {
                if event.state == HotKeyState::Pressed {
                    ...
                    let modifier_sh_tmp = my_app_instance.modifier_sh_tmp.clone();
                    let code_sh_tmp = my_app_instance.code_sh_tmp.clone();
                    let hotkey_take = HotKey::new(string_to_modifiers(&modifier_sh_tmp), string_to_key(&code_sh_tmp));

                    let modifier_save_tmp = my_app_instance.modifier_save_tmp.clone();
                    let code_save_tmp = my_app_instance.code_save_tmp.clone();
                    let hotkey_save = HotKey::new(string_to_modifiers(&modifier_save_tmp), string_to_key(&code_save_tmp));

                    if event.id == hotkey_take.id() {
                        my_app_instance.show_main_screen = false;
                    ...
                    if event.id == hotkey_save.id() {
                        if let Some(screenshot) = my_app_instance.screenshot.as_ref() {
                            ...
                            let format = &my_app_instance.format_tmp;
...
fn main() {
    ...
    my_app_instance.lock().unwrap().register_hotkey();

    let _ = register_hotkey_listener(Arc::clone(&my_app_instance));
...
impl MyApp {
    fn register_hotkey(&mut self) {
        let modifier_sh_tmp = self.modifier_sh_tmp.clone();
        let code_sh_tmp = self.code_sh_tmp.clone();
        let hotkey_take = HotKey::new(string_to_modifiers(&modifier_sh_tmp), string_to_key(&code_sh_tmp));

        let modifier_save_tmp = self.modifier_save_tmp.clone();
        let code_save_tmp = self.code_save_tmp.clone();
        let hotkey_save = HotKey::new(string_to_modifiers(&modifier_save_tmp), string_to_key(&code_save_tmp));
    ...
    fn unregister_hotkeys(&mut self) {
        let hotkeys_to_unregister: Vec<HotKey> = vec![
            HotKey::new(string_to_modifiers(&self.modifier_sh_tmp), string_to_key(&self.code_sh_tmp)),
            HotKey::new(string_to_modifiers(&self.modifier_save_tmp), string_to_key(&self.code_save_tmp)),
        ];
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.show_main_screen {
            ...
            egui::CentralPanel::default().frame(panel_frame).show(ctx, |ui| {
                ...
                content_ui.horizontal(|ui| {
                    ...
                    if ui.button("Save").clicked() {
                        if let Some(screenshot) = self.screenshot.as_ref() {
                            let raw_data = screenshot.as_raw();
                            let image_buffer =
                                RgbaImage::from_raw(screenshot.width() as u32, screenshot.height() as u32, Vec::from(raw_data));
                            let format = &self.format_tmp;
                            if let Some(img_buffer) = image_buffer.as_ref() {
                                save_image_or_gif(img_buffer, format, "screen").ok();
                            }
                        }
                ...
                let mut image_ui = content_ui.child_ui(
                    image_rect,
            ...
            ctx.show_viewport_immediate(
                ...
                |ctx, _class| {
                    egui::CentralPanel::default().frame(panel_frame).show(ctx, |ui| {
                        ...
                        let button_response =
                            ui.interact(interaction_rect, egui::Id::new("button"), egui::Sense::click());
